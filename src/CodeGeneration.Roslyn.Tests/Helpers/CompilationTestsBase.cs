...
public abstract class CompilationTestsBase
{
    ...
    protected static async Task AssertGeneratedAsExpectedAsync(string source, string expected)
    {
        var generatedTree = await GenerateAsync(source);
        // normalize line endings to just LF
        var text = await generatedTree.GetTextAsync();
        var generatedText = NormalizeToLf(text.ToString());
